			...
			<table class="table table-striped table-bordered table-hover table-condensed">
				...
				<?php if(is_array($data)): foreach($data as $key=>$v): ?><tr>
						...
						<td>
							...
							<a href="javascript:;" sort="<?php echo ($v['order_number']); ?>" CategoryId="<?php echo ($v['id']); ?>" CategoryName="<?php echo ($v['name']); ?>" CategoryMca="<?php echo ($v['mca']); ?>" Desc="<?php echo ($v['desc']); ?>" isShow="<?php echo ($v['is_show']); ?>" CategoryIco="<?php echo ($v['ico']); ?>"
							   CateWidth="<?php echo ($v['show_width']); ?>" ShowHeight="<?php echo ($v['show_height']); ?>" ShowTitle="<?php echo ($v['show_title']); ?>"  onclick="edit(this)">修改</a>
				...
				<form id="bjy-form" class="form-inline" action="<?php echo U('Admin/Category/add');?>" method="post">
					...
					<table class="table table-striped table-bordered table-hover table-condensed">
						...
						<tr>
							<th>前台显示：</th>
							<td>
								宽度<input style="width: 60px;" class="form-control" type="text" name="show_width">% 　　
								高度<input style="width: 60px;" class="form-control" type="text" name="show_height">px
								<p>是否显示标题 　
									<input class="form-control" type="radio" value="1" name="show_title">显示　　
									<input class="form-control" type="radio" value="0" name="show_title">不显示</p>
								<p style="color: #c0c0c0;">显示在前台的宽度,合理设置, 按照百分比,60%, 直接输入60即可,默认20</p>
								<p style="color: #c0c0c0;">显示在前台的高度,合理设置,默认50px,保持一行内高度一致</p>
							</td>
						</tr>
						<tr>
							<th>排序：</th>
				...
				<form id="bjy-form2" class="form-inline" action="<?php echo U('Admin/Category/edit');?>" method="post">
					...
					<table class="table table-striped table-bordered table-hover table-condensed">
						...
						<!--<tr>-->
							<!--<th>前台显示宽度：</th>-->
							<!--<td>-->
								<!--<input class="form-control" type="text" name="show_width">-->
								<!--<p style="color: #c0c0c0;">显示在前台的宽度,合理设置, 按照百分比,60%, 直接输入60即可,默认20</p>-->
							<!--</td>-->
						<!--</tr>-->
						<tr>
...
function edit(obj){
	...
	CategoryIco == "" ? CategoryIco = '/tpl/Public/js/fex/image.png':'';
	$("#bjy-form2 input[name='ico']").next('img').attr('src',CategoryIco);
	$("#bjy-form2 input[name='show_width']").val($(obj).attr('CateWidth'));
	$("#bjy-form2 input[name='show_height']").val($(obj).attr('ShowHeight'));

	if ( $(obj).attr('ShowTitle') == '1' ){
        $("#bjy-form2 input[name='show_title']").eq(0).attr("checked",true);
	}else{
        $("#bjy-form2 input[name='show_title']").eq(1).attr("checked",true);
	}
